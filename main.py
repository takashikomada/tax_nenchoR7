...
def render_sidebar() -> str:
    ...
    with st.sidebar:
        ...
        # 生命保険料控除（新契約のみ）の簡易試算ツール
        st.subheader("生命保険料控除のかんたん試算（新契約・所得税）")
        st.caption(
            "※平成24年1月1日以後に締結した保険契約（新契約）のみを対象とした簡易計算です。"
        )

        with st.expander("控除証明書の金額を入力してください（新契約分）", expanded=False):
            # 一般生命保険料の計算（A）
        ...
        # 結果表示（シンプルに、でも用紙の「計」っぽく）
        st.markdown("**試算結果（所得税の生命保険料控除）**")
        st.markdown(
            f"""
- 一般生命保険料控除：**{gen_ded:,}円**
- 介護医療保険料控除：**{med_ded:,}円**
...
"""
        ...
        st.caption(
            "※旧契約（平成23年12月31日以前）との組合せや、実際の申告書への記入方法は、"
...
def main():
    ...
    try:
        with st.spinner("年末調整の手引きや関連資料を読み込み中...（初回のみ少し時間がかかります）"):
            setup_retriever()
